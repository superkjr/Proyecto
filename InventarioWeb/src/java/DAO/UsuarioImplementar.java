...
public class UsuarioImplementar  implements UsuarioDAO{
    ...
    }//    SELECT COUNT(id)AS cantidad FROM `tb_usuario` WHERE correo='" + usu.getCorreo() + "' AND clave='" + usu.getClave()+"'";

    @Override
    public ArrayList<Usuario> startSesion(String user, String clave) {
        this.conn = FactoryConexionDB.open(FactoryConexionDB.MySQL);
        StringBuilder miSQL = new StringBuilder();
        miSQL.append("SELECT COUNT(id)AS CANTIDAD FROM `tb_usuario` correo = '").append(user);
